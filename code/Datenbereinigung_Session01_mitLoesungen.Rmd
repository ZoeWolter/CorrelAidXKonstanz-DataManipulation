...
Um diese Frage zu beantworten und die Häufigkeit deren Auftretens zu bestimmen, nutzen wir das **naniar** Package. Dieses ist hilfreich für die Zusammenfassung und den Umgang mit NA's in R. Es versucht hierbei so konsistent wie möglich mit den Prinzipien des `tidyverse` zu sein. 
Es ist immer wichtig, sich das Codebook des Datensatzes genau anzuschauen. In diesem erhaltet Ihr Angaben über implizite NA's. Zum Beispiel Werte wie 99, 98 oder -1, -2, die häufig zur Codierung bestimmter fehlender Werte genutzt werden. Diese müssen für die meisten Analysen explizit in NAs umgewandelt werden. Nachdem wir dies getan haben, können wir ihr Auftreten im Datensatz inspizieren. 
...
BKA_preprocessed_gesamt <- BKA_preprocessed %>%
  dplyr::filter(Straftat == "Straftaten insgesamt") %>% 
  dplyr::select(Straftat, 
                Stadt_Landkreis, 
                Kreisart, 
                erfasste_Faelle_Anzahl) %>%
  dplyr::mutate(erfasste_Faelle_neu = case_when(Kreisart != "SK" ~ erfasste_Faelle_Anzahl),
                ...
                Kreisart_neu = replace(Kreisart_neu, 
                                       ...
                                       "Unbekannt")) %>%
  naniar::replace_with_na_at(.vars = c("erfasste_Faelle_neu"),
...
```{r}
BKA_preprocessed_gesamt %>%
  naniar::miss_var_summary()
```

Überprüfen der Variable "Kreisart_neu" mit `summary()`
```{r}
table(BKA_preprocessed_gesamt$Kreisart_neu)
```

Um die NA's explizit zu codieren, können wir zum Beispiel `naniar::replace_with_na()` nutzen oder "Kreisart_neu" in einen *factor* umwandeln und die *factor levels* hierbei angeben.

```{r}
BKA_preprocessed_gesamt <- BKA_preprocessed_gesamt %>% 
  naniar::replace_with_na(replace = list(Kreisart_neu = c("Keine Angabe", "Unbekannt")))

table(BKA_preprocessed_gesamt$Kreisart_neu)
miss_var_summary(BKA_preprocessed_gesamt)
```

```{r}
# Plot mit der Anzahl an NA's pro Variable
naniar::gg_miss_var(BKA_preprocessed_gesamt)

# Visualisierung der Position der NA's im Datensatz
naniar::vis_miss(BKA_preprocessed_gesamt)

# Visualisierung der bei selben Observationen auftretenden NA's
naniar::gg_miss_upset(BKA_preprocessed_gesamt)

# Plots der NA's in Abhängigkeit anderer Variablen
naniar::gg_miss_var(BKA_preprocessed_gesamt, facet = Kreisart)
```


## Haben wir Outlier im Datensatz?

Outlier sind Beobachtungen, die ungewöhnlich sind; Datenpunkte, die nicht in das Muster zu passen scheinen. Manchmal handelt es sich bei Outliern um Fehler bei der Dateneingabe; manchmal deuten Outlier auf wichtige neue wissenschaftliche Erkenntnisse hin.

Als Beispiel betrachten wir die Aufklärungsquote einzelner Straftatsbestände.

```{r}
BKA_preprocessed_einzeln <- BKA_preprocessed %>% 
  dplyr::filter(Straftat != "Straftaten insgesamt")

BKA_preprocessed_einzeln_SK <- BKA_preprocessed %>% 
  dplyr::filter(Straftat != "Straftaten insgesamt",
                Kreisart == "SK") %>% 
  dplyr::mutate(Stadt_Landkreis = replace(Stadt_Landkreis, 
                                          erfasste_Faelle_Anzahl > 3000 &
                                            erfasste_Faelle_Anzahl < 4000, 
                                          "Keine Angabe"),
...
```{r}
summary(BKA_preprocessed_einzeln$Aufklaerungsquote)
```
...
Bevor wir unserer Analyse fortsetzen können, müssen wir das beiligende Codebook nochmals genau anschauen und in Erfahrung bringen, wie es zu diesen Outliern kommen kann.


**AUFGABE**:

a) Nutzt den "BKA_preprocessed_einzeln_SK" Datensatz und überprüft, ob es bei der Variable implizite NA's gibt. Falls dies der Fall ist, nutzt `replace_with_na()` um diese explizit als NA's zu klassifizieren.

```{r}
table(BKA_einzeln_SK$Stadt_Landkreis)

BKA_einzeln_SK <- BKA_einzeln_SK %>% 
  naniar::replace_with_na(replace = list(Stadt_Landkreis = c("Keine Angabe", "Nicht erfasst")))

table(BKA_einzeln_SK$Stadt_Landkreis)
BKA_einzeln_SK %>% 
  glimpse()

table(BKA_preprocessed_einzeln_SK$Stadt_Landkreis)

BKA_preprocessed_einzeln_SK <- BKA_preprocessed_einzeln_SK %>% 
  naniar::replace_with_na(replace = list(Stadt_Landkreis = c("Keine Angabe", "Nicht erfasst")))

table(BKA_preprocessed_einzeln_SK$Stadt_Landkreis)
BKA_preprocessed_einzeln_SK %>% 
  glimpse()
```
...
```{r}
BKA_preprocessed_einzeln_SK$Stadt_Landkreis <- BKA_preprocessed_einzeln_SK$Stadt_Landkreis %>% 
  factor()
BKA_preprocessed_einzeln_SK %>% 
  glimpse()
```
...
```{r}
summary(BKA_preprocessed_einzeln_SK$Aufklaerung_Anzahl)
