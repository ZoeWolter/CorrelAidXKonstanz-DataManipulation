...
Die Funktion `here::here()` spielt uns den Pfad zum Ordner zurück in dem unser Projektfile liegt. Von diesem Ordner ausgehend können wir dann einen
Pfad zum Datensatz bilden.
...
Mit der Pipe kann das Programmieren intuitiver gestaltet werden. 
Um die Pipe nutzen zu können, muss das package `magritr` geladen sein. Dieses ist Teil 
des `tidyverse` und wird automatisch geladen wenn wir `library(tidyverse)` ausführen.
...
Das BKA_preprocessed stellt ihre Fallstatistiken auch im csv-format zum Downloaden bereit.
...
```{r}
filter(BKA_preprocessed, Straftat == "Straftaten insgesamt", erfasste_Faelle_Anzahl > 8000)
```
...
```{r}
BKA_preprocessed %>% 
  arrange(desc(Schusswaffe_gedroht)) # was passiert wohl ohne desc(ending)?
```
...
```{r}
BKA_preprocessed %>% 
  select(starts_with("TV_")) %>% # nützlich um sich tippen zu sparen 
  mutate(TV_anteil_maennlich = (TV_maennlich / TV_insgesamt)*100,
       TV_anteil_weiblich = (TV_weiblich / TV_insgesamt)*100, 
       TV_anteil_insgesamt = TV_anteil_maennlich + TV_anteil_weiblich)

BKA_preprocessed %>% 
  select(Aufklaerungsquote, Stadt_Landkreis) %>% 
  mutate(Aufklaerungsproportion = Aufklaerungsquote / 100) %>% 
  arrange(desc(Aufklaerungsproportion)) # this is sus
...
```{r}
BKA_preprocessed %>% 
  mutate(across(.cols = starts_with("TV"), ~ .x + 10000))
```
...
Datensätze (data frames in R) können Variablen (=Spalten im Datensatz) mit verschiedenen Typen abbilden. Um die Variablen zu bearbeiten (manipulieren) ist es unabdingbar, deren type zu kennen. Mittelwert von ner Variable
mit Namen zu berechnen gibt nämlich nur semi-Sinn.
...
Hier fokussieren wir uns auf die Variable Kreisart und wandeln diese zunächst in einen factor um. Hier könnt entweder eine list mit den gültigen levels erstellen und diese dem levels Argument zuwesen (`factor(x, levels = list)`) oder dieses Argument nicht spezifizieren, dann werden die levels in alphabetischer Reihenfolge aus den Daten entnommen.

```{r}
BKA_preprocessed$Kreisart <- BKA_preprocessed$Kreisart %>% factor()
